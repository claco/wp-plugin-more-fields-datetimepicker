  ...
  $.ui.ddmanager = {
    ...
    drop: function (draggable, e) {
      ...
      $.each($.ui.ddmanager.droppables[draggable.options.scope], function () {
        ...
        if (!this.options.disabled && this.visible && $.ui.intersect(draggable, this, this.options.tolerance)) dropped = this._drop.call(this, e);
...
(function ($) {
  $.widget("ui.colorpicker", {
    ...
    _downIncrement: function (e) {
      var field = $(e.target).parent().find('input').focus(),
        self = this;
...
(function ($) {
  ...
  $.widget("ui.dialog", {
    _init: function () {
      ...
      var self = this,
        options = this.options,
        uiDialogContent = this.element.removeAttr('title').addClass('ui-dialog-content').wrap('<div/>').wrap('<div/>'),
        uiDialogContainer = (this.uiDialogContainer = uiDialogContent.parent()).addClass('ui-dialog-container').css({
        position: 'relative',
        width: '100%',
        height: '100%'
      }),
        uiDialogTitlebar = (this.uiDialogTitlebar = $('<div/>')).addClass('ui-dialog-titlebar').append('<a href="#" class="ui-dialog-titlebar-close"><span>X</span></a>').prependTo(uiDialogContainer),
        title = options.title || '&nbsp;',
  ...
  $.widget("ui.slider", {
    ...
    _translateRange: function (value, axis) {
      if (this.rangeElement) {
        if (this.currentHandle[0] == this.handle[0] && value >= this._translateValue(this.value(1), axis)) value = this._translateValue(this.value(1, axis) - this._oneStep(axis), axis);
        if (this.currentHandle[0] == this.handle[1] && value <= this._translateValue(this.value(0), axis)) value = this._translateValue(this.value(0, axis) + this._oneStep(axis), axis);
      }
    ...
    _translateLimits: function (value, axis) {
      if (value >= this.actualSize[axis == "x" ? "width" : "height"] - this._handleSize(null, axis)) value = this.actualSize[axis == "x" ? "width" : "height"] - this._handleSize(null, axis);
      if (value <= 0) value = 0;
      return value;
    },
    ...
    _stop: function (e) {
      this._propagate('stop', e);
      if (this.firstValue != this.value()) this._propagate('change', e);
...
(function ($) {
  $.widget("ui.tabs", {
    ...
    _tabify: function (init) {
      ...
      if (o.cookie) $.cookie('ui-tabs-' + $.data(self.element[0]), o.selected, o.cookie);
      ...
      this.$tabs.unbind('.tabs').bind(o.event, function () {
        ...
        if (o.cookie) $.cookie('ui-tabs-' + $.data(self.element[0]), self.options.selected, o.cookie);
        self.$panels.stop();
        if ($show.length) {
          var a = this;
...
(function ($) {
  ...
  $.extend(Datepicker.prototype, {
    ...
    _showTime: function(id) {
        var target = $(id);
        var inst = this._getInst(target[0]);
        return this._get(inst, 'showTime');
    },
    _selectTime: function(id, hour, minutes, ampm, select) {
        var target = $(id);
        var inst = this._getInst(target[0]);
        
        inst.selectedHour = parseFloat($(select).parent().find('.ui-datepicker-new-hour').val());
        inst.selectedMinute = parseFloat($(select).parent().find('.ui-datepicker-new-minutes').val());
        inst.selectedAmPm = $(select).parent().find('.ui-datepicker-new-ampm').val();
        
        if (inst.selectedAmPm == 'PM' && inst.selectedHour < 12) {
            inst.selectedHour += 12;
        } else if (inst.selectedAmPm == 'AM' && inst.selectedHour == 12) {
            inst.selectedHour = 0;
        }
    ...
    formatDate: function (format, date, settings) {
      ...
      if (date) for (var iFormat = 0; iFormat < format.length; iFormat++) {
        ...
        case 'g':
            ...
            output += formatNumber('d', hours > 12 ? hours - 12 : hours , 2);
            break;
        case 's':
            minutes = formatNumber('d', date.getMinutes(), 2);
    ...
    _setDateFromField: function (inst) {
      ...
      if (dates.length > 0) {
        ...
        var ampm = dates[0].substring(dates[0].length - 2).toUpperCase();
        if (ampm == 'AM' || ampm == 'PM') {
            var minutes = parseInt(dates[0].substring(dates[0].length - 4,  dates[0].length - 2));
            var hours = parseInt(dates[0].substring(dates[0].length - 7,  dates[0].length - 5));
    ...
    _generateMonthYearHeader: function (inst, drawMonth, drawYear, minDate, maxDate, selectedDate, secondary, showStatus, initStatus, monthNames) {
      ...
      if (secondary || !this._get(inst, 'changeMonth')) monthHtml += monthNames[drawMonth] + '&#xa0;';
        ...
        for (var month = 0; month < 12; month++) {
          if ((!inMinYear || month >= minDate.getMonth()) && (!inMaxYear || month <= maxDate.getMonth())) monthHtml += '<option value="' + month + '"' + (month == drawMonth ? ' selected="selected"' : '') + '>' + monthNames[month] + '</option>';
        }
        monthHtml += '</select>';
      }
      ...
      if (secondary || !this._get(inst, 'changeYear')) html += drawYear;
        ...
        if (drawHour   > 11) { drawAmPm = "PM";        }
        if (drawHour   > 12) { drawHour = drawHour - 12; }
        if (drawHour   == 0) { drawHour = 12;        }
      ...
      if (showTime) {
          ...
          // Hour
          html += '<select class="ui-datepicker-new-hour" onchange="jQuery.datepicker._selectTime(\'#' + inst.id + '\', ' + drawHour + ', ' + drawMinute + ', \'' + drawAmPm + '\', this);">';
          for (; hour <= endHour; hour++) {
            html += '<option value="' + hour + '"' + (hour == drawHour ? ' selected="selected"' : '') + '>' + hour + '</option>';
          }
          html += '</select>';

          // Minutes
          html += '<select class="ui-datepicker-new-minutes" onchange="jQuery.datepicker._selectTime(\'#' + inst.id + '\', ' + drawHour + ', ' + drawMinute + ', \'' + drawAmPm + '\', this);">';
          for (; minute <= endMinute; minute++) {
            var len = (minute + '').length;
            html += '<option value="' + minute + '"' + (minute == drawMinute ? ' selected="selected"' : '') + '>' + (len == 1 ? '0' + minute : minute) + '</option>';
          }
          html += '</select>';

          // AM/PM
          html += '<select class="ui-datepicker-new-ampm" onchange="jQuery.datepicker._selectTime(\'#' + inst.id + '\', ' + drawHour + ', ' + drawMinute + ', \'' + drawAmPm + '\', this);">';
          html += '<option value="AM"' + (drawAmPm == 'AM' ? ' selected="selected"' : '') + '>AM</option>';
          html += '<option value="PM"' + (drawAmPm == 'PM' ? ' selected="selected"' : '') + '>PM</option>';
          html += '</select>';
    ...
    _formatDate: function (inst, day, month, year) {
      ...
      var formattedDate = this.formatDate(this._get(inst, 'dateFormat'), date, this._getFormatConfig(inst));
    
      if (showTime) {
          date.setHours(inst.selectedHour);
          date.setMinutes(inst.selectedMinute);
    
          var formattedTime = this.formatDate(this._get(inst, 'timeFormat'), date, this._getFormatConfig(inst));
      ...
      },
      {
        ...
        step: function (step, b) {
          ...
          var timestamp = new Date().getTime(),
            elapsedTime = (timestamp - b.startTime);
